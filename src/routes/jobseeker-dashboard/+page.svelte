<script lang="ts">
	...
	onMount(async () => {
		...
		first = $session.user!.displayName!.indexOf('<');
		name = $session.user!.displayName!.substring(0, first);
...
<div class="flex flex-row items-center justify-between p-4">
	...
	<div class="flex h-full flex-row content-center justify-center gap-4">
		<Button
			onclick={() => {
				window.open($session.user!.photoURL!, '_blank');
			}}>View Resume</Button
		>
		<Avatar.Root>
										...
										<Dialog.Root>
											...
											<Dialog.Content>
												<Dialog.Header>
													...
													<br />
													<Button href={job.applylink} target="__blank__">Apply</Button>
												</Dialog.Header>
											</Dialog.Content>
										</Dialog.Root>
									</div>
